...
public class CloneGroupFinder {
  ...
  private CloneGroupFinder(Builder builder) {
  ...
  public static final class Builder {
    ...
    public CloneGroupFinder build() {
      return new CloneGroupFinder(this);
    }
  }
   ...
   public void addSourceFileForDetection(String fileName) {
	...
	public List<Clone> findClones(){
		...
		for(Block block : candidateBlockList){
			//List<Block> sameHashBlockGroup = new ArrayList<Block>(cloneIndex.getBySequenceHash(block.getBlockHash()));
			//Collections.sort(sameHashBlockGroup, new BlockSorter());
			sameHashBlockGroups.add(new ArrayList<Block>(cloneIndex.getBySequenceHash(block.getBlockHash())));
		}
		...
		for(Block block1 : list1){
			...
			for(Block block2 : list2){
				if(block1.getResourceId().equals(block2.getResourceId())
						&& block1.getIndexInFile() >= block2.getIndexInFile()){
	...
	static class BlockSorter implements Comparator<Block>{
		public int compare(Block o1, Block o2) {
			int n = o1.getResourceId().compareTo(o2.getResourceId());
			return n == 0 ? Integer.valueOf(o1.getIndexInFile()).compareTo(Integer.valueOf(o2.getIndexInFile())) : n;
		}
	}
