 ...
 * pop: returns head token and remove it from queue 
 * peek: returns head token without remove it from queue 
 * lookahead: returns a token from the queue with specified index
 * starting from the head without removing any token
 * 
 * @author sharif
 * 
 */
public class TokenQueue {
	...
	/**
	 * Returns a token ahead of the current position, without consume it
	 * The first item to be looked ahead at has index 1.
	 */
	 ...
	 * 
	 * as a successful token matcher consumes token from token queue,
	 * this is required when a statement is build partially by some successful token matchers 
	 * but eventually failed to build in full because of failure of some other token matchers
	 * 
	 * 
